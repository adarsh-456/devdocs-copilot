...
def load_doc_chunks(chunk_size: int = CHUNK_SIZE_WORDS, overlap: int = CHUNK_OVERLAP_WORDS):
    chunks = []
...
def load_issue_chunks(chunk_size: int = CHUNK_SIZE_WORDS, overlap: int = CHUNK_OVERLAP_WORDS):
    chunks = []
    for path in sorted(ISSUES_DIR.glob("*.json")):
        ...
        for i, piece in enumerate(split_text(full_text, chunk_size, overlap)):
            chunks.append({
...
def build_all_chunks(chunk_size: int = CHUNK_SIZE_WORDS, overlap: int = CHUNK_OVERLAP_WORDS):
    doc_chunks = load_doc_chunks(chunk_size, overlap)
    issue_chunks = load_issue_chunks(chunk_size, overlap)
